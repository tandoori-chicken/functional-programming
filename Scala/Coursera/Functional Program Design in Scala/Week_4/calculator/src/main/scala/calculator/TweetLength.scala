...
object TweetLength {
  ...
  def getColorForRemaining(remainingCharsCount: Signal[Int]): String = remainingCharsCount() match{
    case l if l>=15 => "green"
    case l if l>=0 => "orange"
    case _ => "red"
  }

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = Signal(getColorForRemaining(remainingCharsCount))
